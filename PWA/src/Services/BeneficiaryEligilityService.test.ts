...
import { Database, ObjectStoreName } from "./Database";
...
const eligibleActiveDistributionBeneficiary = new Beneficiary(
  activeEligibleBeneficiaryCode,
  ["code", "name"],
  [activeEligibleBeneficiaryCode, "henry"]
);
const eligibleInactiveDistributionBeneficiary = new Beneficiary(
  activeEligibleBeneficiaryCode,
  ["code", "name"],
  [inActiveEligibleBeneficiaryCode, "tedd"]
);
const activeDistribution = new Distribution(
  "items",
  "date",
  "place",
  "activeDistribution"
);
const inactiveDistribution = new Distribution(
  "items",
  "date",
  "place",
...
async function setupDatabase(): Promise<Database> {
  ...
  await database.addBeneficiaryToDistribution(
    eligibleActiveDistributionBeneficiary,
    activeDistribution
  );
  await database.addBeneficiaryToDistribution(
    eligibleInactiveDistributionBeneficiary,
    inactiveDistribution
  );
