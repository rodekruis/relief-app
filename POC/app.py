...
def login_post():
    ...
    return render_template('index_distrib.html', email=current_user.email)#url_for('main.index'))
...
def create_distrib():
    if 'distrib_name' in request.form.keys():
        ...
        if request.form['distrib_name'] == "" or request.form['distrib_place'] == "" or \
                request.form['distrib_date'] == "":
            flash('Specify the name, location and date of the distribution.')
...
def get_list_distrib():
    ...
    for distrib_ in distributions:
        ...
        distrib_list.loc[len(distrib_list), distrib_features.keys()] = distrib_features.values()
    return distrib_list
